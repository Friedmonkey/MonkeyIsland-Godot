...
# Import From FriedGrass
# 
# This script demonstrates how to import transforms from FriedGrass. To use it:
#
...
# 1. At the very top, assign your FriedGrass node.
...
@export var fried_grass: MultiMeshInstance3D
...
func import_sgt(value: bool) -> void:
	var sgt_mm: MultiMesh = fried_grass.multimesh
	var global_xform: Transform3D = fried_grass.global_transform	
	print("Starting to import %d instances from FriedGrass using mesh id %d" % [ sgt_mm.instance_count, assign_mesh_id])
	var time: int = Time.get_ticks_msec()
	get_instancer().add_multimesh(assign_mesh_id, sgt_mm, fried_grass.global_transform)	
